...
2. **運行容器**（必須掛載 `/data` 與 `/app/cert` 目錄，並暴露 `8888` 埠）：
   ```bash
   docker run -d \
...
## 儀表板管理面板 (Dashboard Panel)

本專案啟動時，會同步於 `8888` 埠啟動一個現代化的 Web 管理介面（支援深色玻璃擬態風格）。
- **存取網址**：`http://localhost:8888` 或 `http://<您的伺服器IP>:8888`
- **功能特色**：
  - **音樂背景下載**：一鍵開啟/關閉背景下載器（預設開啟）。
  - **依歌手建立資料夾**：開啟後下載的音檔與歌詞會自動分類在以歌手命名的子資料夾中（預設關閉）。
  - **依專輯建立資料夾**：開啟後下載的音檔與歌詞會自動分類在以專輯命名的子資料夾中（預設關閉）。
  - **即時執行日誌**：面板內置了即時日誌視窗，可實時監控背景下載器的下載與 `ffmpeg` 封裝狀態。
